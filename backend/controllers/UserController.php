    ...
    public function actionCreate()
    {
        ...
        if($model->load(Yii::$app->request->post()))
        {
            ...
            if(!empty($model->userimage))
            {
                ...
                $model->userimage->saveAs(Yii::getAlias('@frontend')."/web/img/uploads/".$filename);
                $model->userimage=$filename;
            }
            if ($user = $model->saveUser()) {
                if(!empty($user_data['UserForm']['role'])){
                    
                    $auth = Yii::$app->authManager;
                    $authorRole = $auth->getRole($user_data['UserForm']['role']);
